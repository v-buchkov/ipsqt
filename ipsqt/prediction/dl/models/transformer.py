...
class TransformerClassifier(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.encoder_layer = nn.TransformerEncoderLayer(
            d_model=n_features + 1,  # +1, as have additional feature = position encoding
        ...
        self.final_layer = nn.Sequential(
            nn.Linear(self.n_features + 1, hidden_size),  # +1 from positional encoding
            nn.ReLU(),
            nn.Dropout(dropout),
    ...
    def forward(
        ...
        out = positional_encoding(x, model_device, 1)

        mask = self.generate_square_subsequent_mask(
            1, device=model_device
        )
